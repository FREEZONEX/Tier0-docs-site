...
```mermaid
flowchart TB
  uns["UNS 주문 데이터<br/>ERP의 주문"]
  production["생산 부서"]

  subgraph app["작업 지시 관리 앱"]
    direction LR
    orders["<b>생산 주문</b><br/><span style='font-size:13px'>(선택 사항) 주문 생성</span>"]
    plans["<b>계획</b><br/><span style='font-size:13px'>주문 분할<br/>생산 일정</span>"]
    details["<b>계획 상세</b><br/><span style='font-size:13px'>생산 진행 상세</span>"]
  end

  uns -- "주문 데이터" --> orders
  orders --> plans
  plans --> details
  plans -- "생산 계획" --> production
  production -- "생산 진행" --> details
...
## 비즈니스 프로세스 이해
<Steps>
1. Tier0에서 **UNS**로 이동해 ERP에서 온 주문 상세 정보를 확인합니다.
