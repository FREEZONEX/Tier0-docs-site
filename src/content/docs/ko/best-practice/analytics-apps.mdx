---
title: 분석 앱 구축
description: Aramco Bowtie 사례(비식별 데이터)를 사용해 Notebook 기반 분석 앱을 구축합니다.
...
Tier0는 Python 기반 고급 데이터 분석에 Marimo Notebook을 사용합니다. 이 가이드는 Bowtie 애플리케이션을 예제 workflow로 사용합니다.

```mermaid
flowchart LR
  collect["Node-RED"] -->|"수집 데이터"| uns[("UNS<br/>모델링")]
...
정유 공정의 부식은 여러 운전 요인의 영향을 받는 복잡한 현상입니다. 이 예제는 공정 데이터를 결합해 부식 발생 가능성을 추정하고 선제 정비를 지원하는 실시간 부식 위험 평가 workflow를 보여줍니다.

## 원천 데이터 가져오기

<Steps>
1. Tier0에서 **UNS**로 이동해 데이터 모델을 가져옵니다.
    
      <details className="notebook-cell">
      <summary>UNS 모델 JSON 보기</summary>
...
2. **Flows**로 이동해 **Source Flow**를 만들고 원시 데이터를 연결한 뒤 **UNS**에 게시합니다.

    *(Node-RED가 데이터를 연결하고 UNS에 게시합니다.)*
    <details className="notebook-cell">
    <summary>Source Flow JSON 보기</summary>
...
1. Tier0에서 **Notebook**으로 이동해 새 notebook을 만듭니다.

2. notebook을 열고 **UNS** 데이터를 분석하기 위해 다음 cell을 추가합니다.

    - 셀 1 - 라이브러리 가져오기 및 설정

      필요한 라이브러리를 가져오고 MQTT broker, topic, runtime 설정을 구성합니다.
      ...
      :::tip[UNS broker 외부 연결]
      외부 client로 UNS broker에 연결하는 경우 Tier0 인증 정보 외에 다음 정보를 사용합니다.
        ```json
        "MQTT TCP": "mqtt://enterprise-new-dev.tier0.dev:1883",
        "WSS": "wss://enterprise-new-dev.tier0.dev:8084/mqtt"
        ```

    - 셀 2 - Bayesian Network 구조 정의

      Bayesian Network topology, 상태 공간, CPT generator, 확률 규칙을 정의합니다.
    ...
    - 셀 3 - Source data 검증 및 이산화

      수신 MQTT payload를 검증하고 연속 process 값을 Bayesian Network 상태로 변환합니다.
    ...
    - 셀 4 - 모델 구축 및 추론 실행
      ...
      현재 snapshot에 대한 Bayesian Network를 구축하고 추론을 실행한 뒤 output payload를 생성합니다.
    ...
    - 셀 5 - MQTT 분석 서비스 실행
      ...
      source topic을 subscribe하고 각 incoming snapshot을 처리한 뒤 Bayesian analysis를 실행하고 result를 publish합니다.
...
3. 모든 cell을 실행한 뒤 **UNS**로 이동해 결과를 확인합니다.

## Bow-tie 앱 구축
<Steps>
1. Tier0에서 **Builder**로 이동합니다.
2. 대화 상자에 애플리케이션 요구 사항을 입력하고 구축을 시작합니다.

    <Aside type="caution" title="반드시 명시할 내용">
      agent에게 UNS 데이터를 사용하라고 알려주고, 구체적인 topic path를 제공해야 합니다. 그렇지 않으면 agent가 올바른 데이터를 표시하는 데 어려움을 겪을 수 있습니다.
    </Aside>
    <Aside type="tip" title="다른 Tier0에서 앱을 구축하는 경우">
      연결 충돌을 피하려면 서로 다른 **clientId**(다른 suffix 추가)를 사용해 UNS broker에 연결합니다.
    </Aside>
    <details className="notebook-cell">
    <summary>Builder prompt 보기</summary>
