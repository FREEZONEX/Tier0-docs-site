---
title: 산업 프로토콜 연결
description: Node-RED로 OPC UA, Modbus 및 일반 프로토콜 장치를 UNS에 연결합니다. 실제 flow JSON reference를 포함합니다.
editions: [edge, cloud, enterprise]
sidebar:
  order: 2
  label: OPC UA / Modbus 연결
---
import { Steps } from '@astrojs/starlight/components';

이 섹션은 OPC UA, Modbus, API 같은 일반 데이터 유형을 예로 들어 **Source Flow**를 통해 데이터 소스를 **UNS**에 연결하는 과정을 설명합니다.
...
<Steps>
1. Tier0에 로그인하고 **Flow**로 이동해 새 **Source Flow**를 만듭니다.
2. flow에 **inject**, **OPC UA Client**, **mqtt out** node를 추가합니다.
3. 순서대로 연결한 뒤 각 node를 double-click해 detail을 설정합니다.
</Steps>
- **inject**
    <Steps>
    1. 왼쪽 아래의 **add**를 클릭해 property entry를 추가합니다.
    2. properties를 아래 항목으로 설정합니다.
...
- **mqtt out**
    ...
    ```
<Steps>
4. flow를 deploy하고 trigger합니다.
</Steps>

### Flow 참조

<details>
<summary>OPC UA flow JSON 보기</summary>
...
### 데이터 흐름 구성
<Steps>
1. flow에 **modbus-read**와 **mqtt out** node를 추가합니다.
2. node를 double-click하고 필요한 정보를 설정합니다.
    - **modbus-read**
        - **Settings**
          - **FC**: Modbus action을 지정하는 function code입니다. 예: FC 3 = holding register 읽기.
          - **Address**: 읽기를 시작할 register address입니다(보통 0부터 시작).
          - **Quantity**:  The number of consecutive registers to read.
          - **Poll Rate**: node가 Modbus device를 polling하는 주기입니다(예: 10초마다).
          - **Server**: 설정된 Modbus server(IP, port, protocol 등)에 대한 reference입니다.
        - **Server**
          - **Host**: Modbus server IP입니다.
          - **Port**: Modbus server port입니다.
    - **mqtt out**
        ```json
        "Server": "<embedded UNS broker>", //same name as the flow
        "Topic": "<UNS model>",
        ```
3. flow를 deploy하고 trigger합니다.
</Steps>

### Flow 참조

<details>
<summary>Modbus flow JSON 보기</summary>
...
### 데이터 흐름 구성
<Steps>
1. flow에 **inject**, **http request**, **mqtt out** node를 추가합니다.
2. node를 double-click하고 필요한 정보를 설정합니다.
    - **http request**
        ```json
        "Method": "<API method>",
...
### Flow 참조
<details>
<summary>API flow JSON 보기</summary>
...
:::tip[함께 알아두면 좋은 내용]
- **UNS**에 publish하기 전에 필요에 따라 **function** node로 data를 정리할 수 있습니다.
- **debug** node를 사용해 현장에서 data를 확인할 수 있습니다.
- node detail은 [Node-RED flows](https://flows.nodered.org/)를 확인하세요.
:::
