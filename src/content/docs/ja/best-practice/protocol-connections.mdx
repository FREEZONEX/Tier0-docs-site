...
sidebar:
  order: 2
  label: OPC UA/Modbus/Rest API 接続
---
...
<Steps>
1. Tier0 にログインし、**Flow** に移動して新しい **Source Flow** を作成します。
2. flow に **inject**、**OPC UA Client**、**mqtt out** ノードを追加します。
3. 順番に接続し、各ノードをダブルクリックして詳細を設定します。
</Steps>
- **inject**
    <Steps>
    1. 左下の **add** をクリックして property entry を追加します。
    2. properties を次の項目に設定します。
...
### データフローの設定
<Steps>
1. flow に **modbus-read** と **mqtt out** ノードを追加します。
2. 各ノードをダブルクリックして必要な情報を設定します。
    - **modbus-read**
        - **Settings**
...
### データフローの設定
<Steps>
1. flow に **inject**、**http request**、**mqtt out** ノードを追加します。
2. 各ノードをダブルクリックして必要な情報を設定します。
    - **http request**
        ```json
        "Method": "<API method>",
