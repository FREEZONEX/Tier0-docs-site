...
Tier0 は Python による高度なデータ分析に Marimo Notebook を使用します。このガイドでは Bowtie アプリケーションをワークフロー例として使用します。

```mermaid
flowchart LR
  collect["Node-RED"] -->|"収集データ"| uns[("UNS<br/>モデリング")]
...
製油所の腐食は、複数の運転要因に影響される複雑なプロセスです。この例では、プロセスデータを組み合わせて腐食発生確率を推定し、予防保全を支援するリアルタイム腐食リスク評価ワークフローを示します。
...
2. **Flows** に移動し、**Source Flow** を作成して生データを接続し、**UNS** に publish します。
    ...
    *(Node-RED がデータを接続し、UNS に publish します。)*
    <details className="notebook-cell">
...
2. notebook を開き、**UNS** のデータを分析するために次の cell を追加します。
    ...
    - セル 1 - ライブラリのインポートと設定
      ...
      必要なライブラリをインポートし、MQTT broker、topic、実行時設定を構成します。
    ...
    - セル 2 - Bayesian Network 構造の定義

      Bayesian Network の topology、状態空間、CPT generator、確率ルールを定義します。
    ...
    - セル 3 - Source data の検証と離散化

      受信した MQTT payload を検証し、連続的な process value を Bayesian Network の状態に変換します。
    ...
    - セル 4 - モデル構築と推論実行
      ...
      現在の snapshot に対して Bayesian Network を構築し、推論を実行して output payload を生成します。
