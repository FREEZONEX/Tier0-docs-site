---
title: 分析アプリの構築
description: "Aramco Bowtie ケース（匿名化データ）を使い、Notebook で分析アプリを構築します。"
...
Tier0 は Python による高度なデータ分析に Marimo Notebook を使用します。このガイドでは Bowtie アプリケーションを例として使用します。

```mermaid
flowchart LR
  collect["Source Flow"] -->|"収集データ"| uns[("UNS<br/>モデル")]
  uns -->|"生データ"| notebook["Notebook"]
  notebook -->|"分析結果"| uns
  uns -->|"分析結果"| builder["App Builder"]
...
製油所の腐食は、複数の運転要因に影響される複雑なプロセスです。この例では、プロセスデータを組み合わせて腐食発生確率を推定し、予防保全を支援するリアルタイム腐食リスク評価を示します。

## 生データを取得する

<Steps>
1. Tier0 で **UNS** に移動し、データモデルをインポートします。
    
      <details className="notebook-cell">
      <summary>UNS モデル JSON を表示</summary>
...
2. **Flows** に移動し、**Source Flow** を作成して生データを接続し、**UNS** に publish します。

    <details className="notebook-cell">
    <summary>Source Flow JSON を表示</summary>
...
## Notebook で分析アプリを構築する

1. Tier0 で **Notebook** に移動し、新しい notebook を作成します。

2. notebook を開き、**UNS** のデータを分析するために次の cell を追加します。

    - セル 1 - 必要な依存関係をインポートし、MQTT broker、topics、実行時設定を構成します。
    ...
    - セル 2 - Bayesian Network の topology、状態空間、CPT generator、確率ルールを定義します。
    ...
    - セル 3 - 受信した MQTT payload を検証し、連続的な process value を Bayesian Network の状態に変換します。
    ...
    - セル 4 - 現在の snapshot に対して Bayesian Network を構築し、推論を実行して output payload を生成します。
    ...
    - セル 5 - source topic を購読し、受信した各 snapshot を処理して Bayesian 分析を実行し、結果を publish します。
    ...
    - セル 6 - MQTT 接続ステータスとサービス統計を表示します。
...
3. すべての cell を実行し、**UNS** に移動して結果を確認します。

## Bow-tie App を構築する
<Steps>
1. Tier0 で **Builder** に移動します。
2. ダイアログにアプリケーション要件を入力し、構築を開始します。

    <Aside type="caution" title="必ず伝えること">
      agent には UNS のデータを使うことを伝え、具体的な topic path も指定してください。指定しないと、agent が正しいデータを表示できない場合があります。
    </Aside>

    <details className="notebook-cell">
    <summary>Builder prompt を表示</summary>

    ```text
        CDU 常圧塔頂腐食向けの、モダンな産業用 Bow-Tie Analysis アプリケーションを構築してください。

        次の UNS topic のみをデータソースとして使用してください。
        ...
        新しい UNS メッセージが届いたら、アプリケーションは自動的に更新される必要があります。

        このアプリケーションは、オペレーターが現在の腐食リスク、関連する脅威、バリア状態、起こり得る結果を理解できるようにしてください。

        次を含む単一ページの dashboard を作成してください。

        1. メインセクションとして Bow-Tie 図を配置する：
        - 左側に脅威を表示
        - 脅威とトップイベントの間に予防バリアを表示
        - 腐食を中央のトップイベントとして表示
        - トップイベントの後に緩和バリアを表示
        - 右側に結果を表示
        ...
        - 現在のリスク状態
        - モデル信頼度
        - normal、developing、confirmed の確率
        - 総腐食リスク

        3. Corrosion_Monitoring から主要値を表示するコンパクトな live process data panel を作成する。chloride、pH、wash water flow、desalter performance、wash water rate、total iron、SAP evidence、injection quill status を含めてください。
        ...
        5. 視覚ステータスルール：
        - 緑は normal または healthy condition
        - 琥珀色/黄色は developing risk または warning condition
        - 赤は confirmed risk、failed barrier、または severe consequence
        - グレーは data unavailable
        - 青は interaction と informational highlight のみに使用

        6. インタラクション：
        - 脅威、バリア、結果を選択したら、現在の状態、関連する process value、短い説明を表示してください。
        - 新しい UNS データが到着したら、インターフェースを自動更新してください。
        - 最新データの timestamp と、組み込み SDK からの connection/subscription status を表示してください。

        デザイン要件：

        アプリケーション全体で、既存の visual design language と styling を維持してください。

        モダンな industrial operations dashboard スタイルを使用してください。
        - pure black ではなく dark blue-gray color palette を使用する。
        - subtle gradient を持つ layered panels を使用する。
        - 16-20px 程度の大きめの角丸を持つ card-based layout を使用する。
        - heavy shadow ではなく thin borders で分離する。
        - soft depth と控えめな glow effects を使用する。
        - 十分な padding、一貫した alignment、きれいな spacing を保つ。
        - body text には IBM Plex Sans を使用する。
        - headings には Space Grotesk を使用する。
        - 明確な visual hierarchy を持つ、コンパクトで情報密度の高い card を使用する。
        - letter spacing を広げた uppercase section labels を使用する。
        - business analytics ではなく、professional refinery / SCADA interface にする。
        - skeuomorphic elements ではなく、flat illustrations と SVG graphics を使用する。
        - smooth で subtle な transitions と hover effects を使用する。

        全体の見た目は clean、modern、technical、operational にしてください。
        ...
        clear hierarchy、compact cards、readable labels、responsive layout を備えた、clean で professional な refinery operations style を使用してください。

        対応する UNS model で定義された payload fields を正確に使用してください。追加フィールド、計算値、mock data を作らないでください。
    ```
