...
description: Building an analytics app with Notebook using the Aramco Bowtie case (sanitized data).
...
Tier0 uses Marimo Notebook for advanced data analysis with Python. This guide uses a Bowtie application as an example workflow.
...
2. Go to **Flows**, create a **Source Flow** to connect raw data and publish it to **UNS**.
...
## Building an Analytics App in Notebook
...
2. Access the notebook, and add the following cells to analyze data from **UNS**.
    ...
    - Cell 1 - Import Libraries & Configuration
      ...
      In addition to the authentication credentials from Tier0, use the following information when connecting to the UNS broker through an external client:
        ```json
...
2. Enter the application requirements in the dialog, and start building.
    ...
    <Aside type="caution" title="What must be stated">
      Remember to tell the agent to use data from UNS. Specific topic paths are required; otherwise, the agent might have trouble displaying the correct data.
    </Aside>
    <Aside type="tip" title="In case you are building apps on another Tier0">
      Use a different **clientId** (add a different suffix) to connect to the UNS broker and avoid client ID conflicts.
    </Aside>
    <details className="notebook-cell">
