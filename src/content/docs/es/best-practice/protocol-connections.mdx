---
title: Conectar protocolos industriales
description: "Conecta OPC UA, Modbus y otros dispositivos de protocolo típicos a UNS con Node-RED, con referencias reales de flow JSON."
editions: [edge, cloud, enterprise]
sidebar:
  order: 2
  label: Conexi?n OPC UA/Modbus/Rest API
---
import { Steps } from '@astrojs/starlight/components';

Esta sección usa tipos de datos comunes, incluidos OPC UA, Modbus y API, como ejemplos para demostrar el proceso de conectar fuentes de datos a **UNS** mediante **Source Flow**.

## OPC UA

### Configurar Data Flow

<Steps>
1. Inicia sesi?n en Tier0, ve a **Flow** y crea un nuevo **Source Flow**.
2. Agrega los nodos **inject**, **OPC UA Client** y **mqtt out** al flow.
3. Con?ctalos en orden y haz doble clic en cada nodo para configurar los detalles.
</Steps>
- **inject**
    <Steps>
    1. Haz clic en **add** en la esquina inferior izquierda para agregar una property.
    2. Configura las properties con los siguientes elementos:
...
- **mqtt out**
    ...
    ```
<Steps>
4. Despliega y ejecuta el flow.
</Steps>

### Referencia del flow

<details>
<summary>Mostrar OPC UA flow JSON</summary>
...
### Configurar Data Flow
<Steps>
1. En el flow, agrega los nodos **modbus-read** y **mqtt out**.
2. Haz doble clic en los nodos y configura la informaci?n requerida.
    - **modbus-read**
        - **Settings**
          - **FC**: c?digo de funci?n que especifica la acci?n Modbus, por ejemplo FC 3 = leer registros holding.
          - **Address**: direcci?n inicial del registro que se va a leer (normalmente empieza en 0).
          - **Quantity**:  The number of consecutive registers to read.
          - **Poll Rate**: frecuencia con la que el nodo consulta el dispositivo Modbus (por ejemplo, cada 10 segundos).
          - **Server**: referencia a un Modbus server configurado (IP, port, protocol, etc.).
        - **Server**
          - **Host**: IP del Modbus server.
          - **Port**: port del Modbus server.
    - **mqtt out**
        ```json
        "Server": "<embedded UNS broker>", //same name as the flow
        "Topic": "<UNS model>",
        ```
3. Despliega y ejecuta el flow.
</Steps>

### Referencia del flow

<details>
<summary>Mostrar Modbus flow JSON</summary>
...
### Configurar Data Flow
<Steps>
1. En el flow, agrega los nodos **inject**, **http request** y **mqtt out**.
2. Haz doble clic en los nodos y configura la informaci?n requerida.
    - **http request**
        ```json
        "Method": "<API method>",
...
### Referencia del flow
<details>
<summary>Mostrar API flow JSON</summary>
...
:::tip[Otros aspectos que conviene saber]
- Puedes arrastrar un nodo **function** para limpiar los datos según sea necesario antes de publicar en **UNS**.
- Puedes usar un nodo **debug** para revisar los datos en el momento.
- Para detalles de nodos, consulta [Node-RED flows](https://flows.nodered.org/).
:::
