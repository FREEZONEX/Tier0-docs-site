...
2. 进入 **数据流程**，创建 **数据连接**，将数据源连接到 UNS 模型。
        - 机械臂
        <details className="notebook-cell">
        <summary>显示机械臂数据流 JSON</summary>
        ...
        - AGV
        <details className="notebook-cell">
        <summary>显示 AGV 数据流 JSON</summary>
        ...
        - 车床 1
        <details className="notebook-cell">
        <summary>显示车床 1 数据流 JSON</summary>
        ...
        - 车床 2
        <details className="notebook-cell">
        <summary>显示车床 2 数据流 JSON</summary>
...
4. 根据需要，将 topic 字段绑定到所选模型节点的 **位置** 和 **旋转** 各轴上。
5. 在 **数据标记** 下，为模型添加静态标签，并绑定 UNS 数据以展示静态信息。
...
## 可视化车间场景
<Steps>
1. 在 Tier0 中进入 **场景** 并新增场景。
2. 从 **实例库** 添加车间中存在的所有必要实例。
3. 将实例模型移动到车间中的对应位置。

    :::tip[如何将模型移动到正确位置]
    - 切换顶部工具来移动、缩放和旋转模型，并调整模型视角和坐标位置。
    - 查看右侧的 **对象**，包含了绑定的 UNS 模型，可协助确认模型位置。
    :::
...
## 外部分享
<Steps>
1. 进入 **模型库**，选择车间中已配置的模型。
2. 选择一个已有实例，生成二维码。
3. 选择分享模式，并将二维码发送给外部人员。
        - **分享模型**：生成一个 Tier0 平台链接，用于展示带有实时 UNS 数据的模型。
        - **分享场景**：生成一个 Tier0 平台链接，用于展示模型所在场景，以及实时 UNS 数据和环境设置。
</Steps>
