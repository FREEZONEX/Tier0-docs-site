...
Tier0 的数字孪生功能用于在逼真的 3D 模型和场景中实时查看 UNS 数据。
本节会把一个车间场景构建出来。该车间包含以下设备：
- 2 台车床
- 1 台在车床之间工作的机械臂
- 1 个进料缓存站和 1 个出料缓存区
- 1 台质量检测设备
...
## 从车间采集数据
将所有数据采集到 **UNS**，包括控制机器动作的数据、表示机器状态的数据，以及车间环境数据。
<Steps>
1. 在 **UNS** 中构建数据模型。
        <details className="notebook-cell">
        <summary>查看 UNS 模型 JSON</summary>
...
2. 使用 **数据连接** 将数据源连接到 UNS 模型。
        - Robot Arm
        <details className="notebook-cell">
        <summary>查看机械臂数据连接 JSON</summary>
        ...
        - AGV
        <details className="notebook-cell">
        <summary>查看 AGV 数据连接 JSON</summary>
        ...
        - Lathe 1
        <details className="notebook-cell">
        <summary>查看车床 1 数据连接 JSON</summary>
        ...
        - Lathe 2
        <details className="notebook-cell">
        <summary>查看车床 2 数据连接 JSON</summary>
...
## 可视化车间场景
<Steps>
1. 在 **模型库** 中导入车间中的模型，将 UNS 数据映射到模型，并创建对应实例。
2. 进入 **场景**，从 **实例库** 将车间中的实例添加到场景中。
3. 移动模型到车间中的对应位置，并设置环境光照等细节。
</Steps>
