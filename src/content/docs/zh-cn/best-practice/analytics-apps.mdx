---
title: 构建分析应用
description: "使用 Aramco Bowtie 案例（脱敏数据），通过 Notebook 构建分析应用。"
...
<Steps>
1. 在 Tier0 中进入 **UNS**，导入数据模型。
    
      <details className="notebook-cell">
      <summary>显示 UNS 模型 JSON</summary>
      ...
      :::tip[For external connections to UNS broker]
      Refer to [Connecting MQTT Clients](../../using-tier0/working-with-uns-data/#connecting-mqtt-clients)
      :::
    
2. 进入 **数据流程**，创建 **数据连接**，连接原始数据并发布到 **UNS**。

    <details className="notebook-cell">
    <summary>显示数据连接 JSON</summary>
...
1. 在 Tier0 中进入 **Notebook**，创建一个新的 notebook。

2. 打开该 notebook，添加以下 cell 来分析 **UNS** 中的数据。
    ...
    - 单元 1 - 导入所需依赖，并配置 MQTT broker、topics 和运行时设置。
      ...
      :::tip[外部连接 UNS broker]
      参考 [连接 MQTT 客户端](../../using-tier0/working-with-uns-data/#连接-mqtt-客户端)
      :::
...
3. 运行所有单元，然后进入 **UNS** 检查结果。

</Steps>
## 构建 Bow-tie 应用
<Steps>
1. 在 Tier0 中进入 **Builder**。
2. 在对话框中输入应用需求，并开始构建。

    <Aside type="caution" title="必须说明的内容">
      请告诉 agent 使用 UNS 中的数据，并提供具体 topic。否则 agent 可能难以展示正确的数据。
    </Aside>
    <Aside type="tip" title="如果你在另一个 Tier0 上构建应用">
      使用不同的 **clientId**（添加不同后缀）连接 UNS broker，避免 client ID 冲突。
    </Aside>

    <details className="notebook-cell">
    <summary>显示 Builder prompt</summary>

    ```text
        为 CDU 常压塔顶腐蚀构建一个现代工业 Bow-Tie Analysis 应用。

        仅使用以下 UNS topic 作为数据源：
        ...
        当新的 UNS 消息到达时，应用必须自动更新。

        该应用应帮助操作员理解当前腐蚀风险、相关威胁、屏障状态以及可能后果。
        ...
        3. 一个紧凑的实时过程数据面板，显示 Corrosion_Monitoring 中的关键值，包括 chloride、pH、wash water flow、desalter performance、wash water rate、total iron、SAP evidence 和 injection quill status。
        ...
        6. 交互：
        - 选择威胁、屏障或后果时，应显示其当前状态、相关过程值和简短说明。
        - 当新的 UNS 数据到达时，自动更新界面。
        - 显示最新数据时间戳，以及内置 SDK 提供的连接/订阅状态。

        设计要求：

        在整个应用中保持现有视觉设计语言和样式。

        使用现代工业运营 dashboard 风格：
        - 使用深蓝灰色调，而不是纯黑色。
        - 使用带有细微渐变的分层面板。
        - 使用卡片式布局，圆角较大，约 16-20px。
        - 使用细边框做分隔，避免厚重阴影。
        - 使用柔和层次感和克制的发光效果。
        - 保持清晰间距、充足内边距和一致对齐。
        - 正文使用 IBM Plex Sans。
        - 标题使用 Space Grotesk。
        - 使用紧凑、信息密集且层级清晰的卡片。
        - 使用增加字距的大写 section label。
        - 呈现专业的炼化厂 / SCADA 界面，而不是商业分析 dashboard。
        - 使用扁平插图和 SVG 图形，避免拟物元素。
        - 使用平滑、细微的过渡和 hover 效果。

        整体外观应清晰、现代、技术化，并适合运营场景。
        ...
        使用清晰、专业的炼化运营风格，保持明确层级、紧凑卡片、可读标签和响应式布局。

        严格使用对应 UNS 模型中定义的 payload 字段。不要虚构额外字段、计算值或 mock 数据。
    ```

    </details>
3. 经过几轮对话，确认应用完成后，部署应用。
4. 进入 **应用中心**，打开应用并检查。
</Steps>
