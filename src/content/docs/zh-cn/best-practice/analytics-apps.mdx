...
Tier0 使用 Marimo Notebook 通过 Python 进行高级数据分析。本指南以 Bowtie 应用作为示例流程。

```mermaid
flowchart LR
  collect["Node-RED"] -->|"采集数据"| uns[("UNS<br/>建模")]
...
炼化腐蚀是一个受多种运行因素影响的复杂过程。本示例展示一个实时腐蚀风险评估流程：结合过程数据估算腐蚀发生概率，并支持主动维护。
...
2. 进入 **Flows**，创建 **Source Flow**，连接原始数据并发布到 **UNS**。

    *(Node-RED 负责连接数据并发布到 UNS。)*
    <details className="notebook-cell">
    <summary>显示 Source Flow JSON</summary>
...
2. 打开该 notebook，添加以下 cell 来分析 **UNS** 中的数据。
    ...
    - 单元 1 - 导入库并配置
      ...
      导入所需库，并配置 MQTT broker、topic 和运行时设置。
    ...
    - 单元 2 - 定义 Bayesian Network 结构

      定义 Bayesian Network topology、状态空间、CPT generator 和概率规则。
    ...
    - 单元 3 - 校验并离散化源数据
      ...
      校验传入的 MQTT payload，并将连续过程值转换为 Bayesian Network 状态。
    ...
    - 单元 4 - 构建模型并执行推理
      ...
      基于当前 snapshot 构建 Bayesian Network，执行推理并生成输出 payload。
...
2. 在对话框中输入应用需求，并开始构建。
    ...
    <Aside type="caution" title="必须说明的内容">
      记得告诉 agent 使用 UNS 中的数据，并提供具体 topic path。否则 agent 可能难以展示正确的数据。
    </Aside>
