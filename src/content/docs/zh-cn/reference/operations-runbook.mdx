...
2. 确认以下项目：
    ...
    - `backend`、`redis`、`emqx`、`sourceflow`、`eventflow`、`marimo` 和 `opcua-server` 都运行在活动应用节点上。
</Steps>
## 应用检查
<Steps>
1. 在活动应用节点上运行以下命令：
    ...
    :::note
    不要绕过 `bin/compose.sh` 直接运行原始 `docker compose` 命令。该包装脚本会加载安装器生成的运行时环境。
    :::
...
```
:::note
EMQX 和 OPC UA Server 固定在活动应用节点上。APP VIP 故障切换只保证 `APP VIP:80` HTTP 入口。协议端口未通过 HAProxy TCP 代理路由，因此该设计不承诺协议入口高可用。
:::
## 数据库检查
...
:::caution
- 旧 primary 不能直接启动回生产拓扑。先停止 PostgreSQL、repmgrd 和 keepalived；移除残留 DB VIP；再从当前新 primary 执行 basebackup，将该节点注册为 standby，最后重新连接 witness。
- 故障切换后，不要直接重新运行完整安装器。inventory 中静态的 primary/standby 定义可能扰动当前拓扑。
:::
## 备份与恢复
...
- 恢复：
    ...
    ```
:::note
恢复前确认当前 primary。恢复后运行 `verify.sh`、smoke checks，并实际验证默认用户路径的平台登录。
:::
## 清理与重测
...
```
:::caution
完整清理会删除 6 台节点上的 Tier0 运行时数据、Docker/Swarm 数据和数据库备份。它还会在控制容器退出后对控制节点执行最终清理。仅在确认可丢弃的测试环境中使用该入口。如果需要保留客户数据或事故现场，请使用普通 `--cleanup`，并先完成备份。
:::
## 常见问题
...
```
:::note
确认 `shared_preload_libraries` 包含 `repmgr`，并且 systemd unit 使用 `repmgrd --daemonize=false`。
:::
### 物料校验失败
...
```
:::caution
不要跳过 SHA256 校验继续部署。请重新获取交付包，或按可追溯流程重新构建物料。
:::
